...
public partial class FormDocuments_ConsultaDocumentos_FrmRechazosSunat : BasePage
{
    ...
    {
        RechazossunatBE servicioDocument = new RechazossunatBE();
        ...
        totalRecords = servicioDocument.ReporteRechazoscantidadDA(CodigoEmisor, FInicial, FFinal, tipodoc);
        ...
        List<RechazossunatBE> lRepDocumento = servicioDocument.ListarReporteRechazosDA(CodigoEmisor, FInicial, FFinal, tipodoc, pageSize, idx).ToList();
    ...
    public void btngenera_Click(object sender, EventArgs e)
    {
        ...
        RechazossunatBE servicioDocument = new RechazossunatBE();
        List<RechazossunatBE> lRepDocumento = servicioDocument.ListarReporteRechazosEXCELDA(CodigoEmisor, _fechaInicial, _FechaFinal, tipodoc).ToList();
        List<RechazossunatBE> lRepDocumentoresumen = servicioDocument.ListarReporteRechazosrESUMIDODA(CodigoEmisor, _fechaInicial, _FechaFinal, tipodoc).ToList();

        String fileName = "Reporte Documentos Rechazados por sunat del " + _fechaInicial + " hasta " + _FechaFinal + ".xls";
        ...
        using (StringWriter sw = new StringWriter())
        {
            using (HtmlTextWriter htw = new HtmlTextWriter(sw))
            {
                ...
                if (CodigoEmisor.Trim() == "AG0000000031")
                {
                    dato = String.Format("<img src='{0}'/><br><br><br><br><br><div style='font-weight: bold; font-size: 26pt; text-align: center;'>{1}<br><p style='font-weight: bold; font-size: 10pt; text-align: right;'>{2}</p></div>", WebHelper.getProtocoloUrl() + "/gogdn/library/img/carlsonlogo1.jpg", "Reporte Documentos rechazados por Sunat del " + _fechaInicial + " hasta " + _FechaFinal, "Reporte EsavDoc R2.4");
                }
                if (CodigoEmisor.Trim() == "OT0000000005")
                {
                    dato = String.Format("<img src='{0}'/><br><br><br><br><br><div style='font-weight: bold; font-size: 26pt; text-align: center;'>{1}<br><p style='font-weight: bold; font-size: 10pt; text-align: right;'>{2}</p></div>", WebHelper.getProtocoloUrl() + "/gogdn/library/img/rokylogo.jpg", "Reporte Documentos rechazados por Sunat del " + _fechaInicial + " hasta " + _FechaFinal, "Reporte EsavDoc R2.4");
                }
            ...
            using (HtmlTextWriter htw1 = new HtmlTextWriter(sw))
            {
                ...
                if (CodigoEmisor.Trim() == "AG0000000031")
                {
                    dato = String.Format("<div style='font-weight: bold; font-size: 30pt; text-align: left;'>{0}<br><p style='font-weight: bold; font-size: 10pt; text-align: right;'>{1}</p></div>", "Resumen Totales", "                    Reporte EsavDoc R2.4");
                }
                if (CodigoEmisor.Trim() == "OT0000000005")
                {
                    dato = String.Format("<div style='font-weight: bold; font-size: 30pt; text-align: left;'>{0}<br><p style='font-weight: bold; font-size: 10pt; text-align: right;'>{1}</p></div>", "Resumen Totales", "                    Reporte EsavDoc R2.4");
    ...
    public TableRow WriteHeader()
    {
        ...
        rowHeader.Cells.Add(WriteCellStyle("TIPO DOCUMENTO", 100, dicStyle));
        rowHeader.Cells.Add(WriteCellStyle("DOCUMENTO", 150, dicStyle));
        rowHeader.Cells.Add(WriteCellStyle("SUCURSAL", 90, dicStyle));
        rowHeader.Cells.Add(WriteCellStyle("CODIGO RECHAZO", 90, dicStyle));
        rowHeader.Cells.Add(WriteCellStyle("DESCRIPCION", 800, dicStyle));
        rowHeader.Cells.Add(WriteCellStyle("FECHA EMISION", 170, dicStyle));
        rowHeader.Cells.Add(WriteCellStyle("UDUARIO", 80, dicStyle));
    ...
    public TableRow WriteHeaderresumen()
    {
        ...
        rowHeader.Cells.Add(WriteCellStyle("DOC. RECHAZADOS", 80, dicStyle));
    ...
    private void WriteItem(List<RechazossunatBE> lreportes, Table table)
    {
        ...
        foreach (var item in lreportes)
        {
            ...
            dicStyle13.Add("font-weight", "bold");
                ...
                rowItem.Cells.Add(WriteCellStyle(item.NroDocumento, dicStyle12));
                rowItem.Cells.Add(WriteCellStyle(item.Sucursal, dicStyle));
                rowItem.Cells.Add(WriteCellStyle(item.Descripcion, dicStyle));
                rowItem.Cells.Add(WriteCellStyle(item.FechaEmision, dicStyle));
                rowItem.Cells.Add(WriteCellStyle(item.CodigoRechazo, dicStyle12));
                rowItem.Cells.Add(WriteCellStyle(item.UsuarioCreo, dicStyle));
    ...
    private void WriteIteRESUMEN(List<RechazossunatBE> lreportes, Table table)
    {
        TableRow rowItem = new TableRow();
        ...
        foreach (var item in lreportes)
        {
            ...
            if (corre != cant)
            {
                ...
                rowItem.Cells.Add(WriteCellStyle(item.Sucursal, dicStyle));
                rowItem.Cells.Add(WriteCellStyle(item.TipoDocumento, dicStyle12));
                rowItem.Cells.Add(WriteCellStyle(Convert.ToString(item.cantidad), dicStyle));
            }
            else
            {

                rowItem.Cells.Add(WriteCellStyle(item.Sucursal, dicStyle13));
                rowItem.Cells.Add(WriteCellStyle(item.TipoDocumento, dicStyle13));
                rowItem.Cells.Add(WriteCellStyle(Convert.ToString(item.cantidad), dicStyle13));
            }
            _isBackground = !_isBackground;
            table.Rows.Add(rowItem);
