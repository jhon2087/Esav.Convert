...
public partial class FormDocuments_InterfaceContable_FrmDocElectronico : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            CargarDatosBE servicioDocument = new CargarDatosBE();
            List<CargarDatosBE> DDLlistarseries = servicioDocument.ListarSeriesDA(idemisor, SERIE).ToList();
            ...
            cargarGrilla(DateTime.MinValue.AddYears(1900), DateTime.MinValue.AddYears(1900), "", 1,"0");
        }
      
    }


    private void cargarGrilla(DateTime _fechaInicial, DateTime _FechaFinal, string tipodoc, int idx,string serie)
    {
        ...
        totalRecords = servicioDocument.obtenerRepDocumentosElectronicosFilasDA(CodigoEmisor, FInicial, FFinal, tipodoc, serie);
    ...
    public void cargarGrillas(string _fechaInicial, string _FechaFinal, string tipodoc, string idx,string serie)
    {
        try
        {
            ...
            cargarGrilla(fechaini, fechafin, tipodoc, curPage,serie);
 ...
 public void btngenera_Click(object sender, EventArgs e)
    {
        //Literal4.Text = "Se esta Generando su documento espere";
     ...
     string idemisor = CodigoEmisor.Trim();
     string mes = string.Format("{0:00}", System.DateTime.Today.Month);
     string dia = string.Format("{0:00}", System.DateTime.Today.Day);
     string aniochar = string.Format("{0:0000}", System.DateTime.Today.Year);
     string fechaconvertido = aniochar+ mes + dia;
     //  OBTENGO CORRELATIVO DEL REPORTE R2.1
     CargarDatosBE serviciorep = new CargarDatosBE();
     int corre = serviciorep.obtenerCorrelativoReportesDA(idemisor, "R21", fechaconvertido);
     string nombrearchivo = idemisor + "R2.1" + fechaconvertido + corre;
     //   GENERO XLS DEL REPORTE
     RepConcBE servicioDocument = new RepConcBE();
     servicioDocument.generareportedocemitidosexcelDA(CodigoEmisor, _fechaInicial, _FechaFinal, tipodoc, _serie, nombrearchivo);

    String rutaAplicacion = "T:";
    rutaAplicacion = rutaAplicacion.Replace("\\\\", "\\");
  

    String dlDir = @"\\" + Ruc.Trim()+"\\";
    String path = string.Format(rutaAplicacion + dlDir + "{0}.xls", nombrearchivo);

    int secue = 1; int seg = 0; int pases = 0;
    ...
    while (secue == 1)
    {
        if (System.IO.File.Exists(path))
        {
            ...
            Literal4.Text = "El Documento xls ya esta generado";

            string urldescargar = string.Format(WebHelper.getProtocoloUrl() + "/FormDocuments/Comun/FrmDescargar.aspx?hdNomReporte={0}&hdruc={1}&hdemisor={2}&hdfconvert={3}", nombrearchivo, Ruc.Trim(), CodigoEmisor.Trim(), fechaconvertido);
            btndescargar.Attributes.Add("onClick", string.Format("lnkDescargar_onClick('{0}');", urldescargar));
        ...
        else
        {
            ...
            {
                seg = seg + 2;
                Literal4.Text = "Generando Documento " + seg + "%";
                pases = 0;
     ...
     //List<RepConcBE> lRepDocumento = servicioDocument.ObtenerreportedocemitidosexcelDA(CodigoEmisor, _fechaInicial, _FechaFinal, tipodoc, _serie).ToList();
     //List<RepConcBE> lRepDocumentoresumen = servicioDocument.ObtenerreportedocemitidosresumenexcelDA(CodigoEmisor, _fechaInicial, _FechaFinal, tipodoc, _serie).ToList();
