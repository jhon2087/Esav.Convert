...
public partial class FormDocuments_Comun_FrmListaTipoCambio : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string urlProceso = string.Format(WebHelper.getProtocoloUrl() + "/FormDocuments/Comun/FrmNuevoTipoCambio.aspx", 0);
        btnNuevo.Attributes.Add("onClick", string.Format("lnkNuevo_onClick('{0}');", urlProceso));
        string fecha = "1900-01-01";
        cargarGrilla("0", Convert.ToDateTime(fecha));
    }



    private void cargarGrilla(string idx, DateTime _fecha)
    {
        ...
        totalRecords = servicioDocument.obtenerTipoCambioCriteriosFilasSE(fecha);
        ...
        List<wswsDocument.TipoCambioBE> lRango = servicioDocument.obtenerTipoCambioCriteriosSE(fecha, pageSize, curPage - 1).ToList();
        rptListado.DataSource = lRango;
        rptListado.DataBind();
    ...
    public void cargarGrillas(string idx, string _fechas)
    {
        try
        {
            curPage = Convert.ToInt32(idx);
            cargarGrilla(idx, Convert.ToDateTime(_fechas));
    ...
    protected void rptListado_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
        {
            ...
            wswsDocument.TipoCambioBE objSucursales = (wswsDocument.TipoCambioBE)e.Item.DataItem;
